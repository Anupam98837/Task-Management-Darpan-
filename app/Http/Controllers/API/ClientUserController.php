...
class ClientUserController extends Controller
{
    ...
    private function logActivity(
        ...
        try {
            ...
            Log::error('user_data_activity_log insert failed (ClientUser)', ['error' => $e->getMessage()]);
        }
    }
    ...
    private function logAuthActivity(
        ...
        try {
            DB::table('user_data_activity_log')->insert([
                'performed_by'      => $clientUserId ?: 0,
                'performed_by_role' => 'client_user',
                'ip'                => $request->ip(),
                'user_agent'        => (string) $request->userAgent(),
                'activity'          => $activity,
                'module'            => 'Auth',
                'table_name'        => 'client_users',
                'record_id'         => $clientUserId,
                'changed_fields'    => null,
                'old_values'        => $oldValues ? json_encode($oldValues, JSON_UNESCAPED_UNICODE) : null,
                'new_values'        => $newValues ? json_encode($newValues, JSON_UNESCAPED_UNICODE) : null,
                'log_note'          => $note,
            ...
            Log::error('user_data_activity_log insert failed (ClientUser auth)', ['error' => $e->getMessage()]);
        }
    }

    private function decorateClientUser(?object $row): ?object
    {
        if (!$row) {
            return null;
        }

        $assigned = DB::table('client_user_clients as cuc')
            ->join('clients as c', 'c.id', '=', 'cuc.client_id')
            ->leftJoin('clients as p', 'p.id', '=', 'c.parent_id')
            ->where('cuc.client_user_id', $row->id)
            ->orderBy('c.name')
            ->get([
                'c.id',
                'c.name',
    ...
    private function syncClientAssignments(int $clientUserId, array $clientIds): void
    {
        $clientIds = array_values(array_unique(array_map('intval', $clientIds)));

        DB::table('client_user_clients')->where('client_user_id', $clientUserId)->delete();
        ...
        DB::table('client_user_clients')->insert($rows);
    }
    ...
    public function me(Request $request)
    {
        try {
            ...
            $row = DB::table('personal_access_tokens')
                ->where('token', $hashed)
                ->where('tokenable_type', 'client_user')
                ->first();
            ...
            $user = $this->decorateClientUser(
                DB::table('client_users')->where('id', $row->tokenable_id)->first()
            );

            if (!$user) {
                return response()->json(['status' => 'error', 'message' => 'Client user not found'], 404);
            }

            return response()->json(['status' => 'success', 'data' => $user], 200);
        } catch (\Throwable $e) {
            Log::error('[ClientUserController@me] failed', ['error' => $e->getMessage()]);
            return response()->json(['status' => 'error', 'message' => 'Failed to fetch client user info'], 500);
        }
    }
    ...
    public function index(Request $request)
    {
        ...
        $items = $query
            ...
            ->get()
            ->map(fn ($row) => $this->decorateClientUser($row));

        return response()->json([
            'status'  => 'success',
            'message' => 'Client users fetched',
            'data'    => $items,
            'meta'    => [
                'page'        => $page,
                'per_page'    => $perPage,
    ...
    public function show(Request $request, int $id)
    {
        ...
        $row = $this->decorateClientUser(
            DB::table('client_users')->where('id', $id)->first()
        );
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $id = DB::table('client_users')->insertGetId([
                'name'           => $data['name'],
            ...
            DB::rollBack();
            Log::error('[ClientUserController@store] failed', ['error' => $e->getMessage()]);
            return response()->json(['status' => 'error', 'message' => 'Failed to create client user'], 500);
        }

        $fresh = $this->decorateClientUser(DB::table('client_users')->where('id', $id)->first());
        $this->logActivity(
            $request,
            'store',
            'Client Users',
            'Created client user "' . $data['name'] . '"',
            'client_users',
            (int) $id,
            array_keys($data),
            null,
    ...
    public function update(Request $request, int $id)
    {
        ...
        $existing = DB::table('client_users')->where('id', $id)->first();
        if (!$existing) {
            return response()->json(['status' => 'error', 'message' => 'Client user not found'], 404);
        }
        ...
        $data = $request->validate([
            'name'           => ['required', 'string', 'max:160'],
            'email'          => ['required', 'email', 'max:255', Rule::unique('client_users', 'email')->ignore($id)],
            'password'       => ['nullable', 'string', 'min:8'],
            'contact_number' => ['nullable', 'string', 'max:32'],
            'address'        => ['nullable', 'string', 'max:255'],
            'role'           => ['nullable', 'string', 'max:120'],
        ...
        DB::beginTransaction();
        try {
            DB::table('client_users')->where('id', $id)->update($update);
            $this->syncClientAssignments($id, $data['client_ids']);
            DB::commit();
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('[ClientUserController@update] failed', ['error' => $e->getMessage()]);
            return response()->json(['status' => 'error', 'message' => 'Failed to update client user'], 500);
        }
        ...
        return response()->json(['status' => 'success', 'message' => 'Client user updated', 'data' => $fresh]);
    }
    ...
    public function toggle(Request $request, int $id)
    {
        ...
        $row = DB::table('client_users')->where('id', $id)->first();
        if (!$row) {
            return response()->json(['status' => 'error', 'message' => 'Client user not found'], 404);
        }
        ...
        if ($current === 'archived') {
            return response()->json(['status' => 'error', 'message' => 'Archived client users cannot be toggled'], 422);
        }

        $next = $current === 'active' ? 'inactive' : 'active';
        DB::table('client_users')->where('id', $id)->update([
            'status'     => $next,
            'updated_at' => now(),
        ]);
        ...
        $this->logActivity(
            ...
            'toggle',
            'Client Users',
            'Toggled client user "' . ($row->name ?? ('#' . $id)) . '" to ' . $next,
            'client_users',
            $id,
            ['status'],
            (array) $row,
    ...
    public function destroy(Request $request, int $id)
    {
        ...
        $row = $this->decorateClientUser(DB::table('client_users')->where('id', $id)->first());
        if (!$row) {
            return response()->json(['status' => 'error', 'message' => 'Client user not found'], 404);
        }

        DB::table('client_users')->where('id', $id)->delete();

        $this->logActivity(
            $request,
            'destroy',
            'Client Users',
            'Deleted client user "' . ($row->name ?? ('#' . $id)) . '"',
            'client_users',
            $id,
            null,
            (array) $row,
        ...
        return response()->json(['status' => 'success', 'message' => 'Client user deleted']);
    }
}
