{{-- resources/views/expense_heads/create.blade.php --}}
@section('content')
<div class="container py-3">

  <!-- centered panel -->
  <div class="card doc-panel">
    <h2 class="doc-title">Add New Expense Head</h2>
    <p class="doc-sub">Create a new expense head used to categorize expenses.</p>
    ...
    <form id="expenseHeadForm" autocomplete="off" novalidate>
      ...
      <!-- Actions -->
      <div class="actions">
        <a href="{{ url('admin/expenseHead') }}" class="btn btn-secondary" style="display:none">Cancel</a>
...
<script>
  (function(){
    const API_BASE = '/api/expense-heads';
    ...
    document.getElementById('expenseHeadForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (res.status === 201 || res.ok) {
          toast('success', body.message || 'Expense head created');
          // redirect to list (adjust URL if your resource list is under admin area)
          return setTimeout(() => location.href = "{{ url('admin/expenseHead/manage') }}", 400);
        }
