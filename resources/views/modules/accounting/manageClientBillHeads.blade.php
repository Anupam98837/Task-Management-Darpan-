...
<div class="doctypes-page">
  ...
  <div class="toolbar">
    <div class="search-box">
      ...
      <input id="searchInput" type="text" placeholder="Search bill heads by title..." aria-label="Search bill heads">
    </div>
  ...
  <div id="emptyState" class="empty-state" style="display: none;">
    ...
    <h3>No bill head data found</h3>
    <p>Try adjusting your filters or search query</p>
  </div>
    <div class="pagination">
      <div class="pagination-info" id="paginationInfo">
        Showing 1-10 of 0 bill heads
      </div>
...
<!-- Edit Client Bill Head Modal -->
<div class="modal fade" id="editHeadModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">
          <i class="fa-regular fa-pen-to-square me-2"></i>Edit Client Bill Head
        </h5>
...
(function() {
  ...
  if (!els.tbody) {
    console.warn('Client Bill Heads: tbody#rows not found in DOM');
    return;
  }
  ...
  async function fetchExpenseHeads() {
    ...
    try {
      const res = await fetch(`${API_BASE}/client-bill-heads?${params}`, { headers: headersBase, signal });
      ...
      console.error('Fetch client bill heads error', err);
      Swal.fire({ icon:'error', title:'Unable to fetch bill heads', text: String(err.message) });
      els.tbody.innerHTML = `
        <tr>
          <td colspan="6">
            <div class="empty-state">
              <h3>Error loading</h3>
              <p>Unable to fetch bill heads. Try again.</p>
            </div>
          </td>
        </tr>
      `;
...
function render() {
  ...
  function renderPagination(){
    ...
    els.paginationInfo.textContent = `Showing ${state.total ? start : 0}-${state.total ? end : 0} of ${state.total || 0} bill heads`;
  ...
  document.body.addEventListener('change', async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/client-bill-heads/${encodeURIComponent(id)}/toggle-status`, { method:'PATCH', headers: headersBase });
  ...
  document.addEventListener('click', async (e) => {
    ...
    if (!data) {
      try {
        const res = await fetch(`${API_BASE}/client-bill-heads/${encodeURIComponent(id)}`, { headers: headersBase });
        const json = await res.json();
        if (!res.ok) throw new Error(json?.message || 'Request failed');
        data = json?.data || null;
      } catch (err) {
        Swal.fire({ icon:'error', title:'Unable to load bill head', text: String(err.message) });
        return;
      }
    }
  ...
  if (f.form) {
    f.form.addEventListener('submit', async (e) => {
      ...
      try {
        const res = await fetch(`${API_BASE}/client-bill-heads/${encodeURIComponent(id)}`, {
          method: 'PUT',
  ...
  if (els.exportBtn) {
    els.exportBtn.addEventListener('click', () => {
      ...
      const a = document.createElement('a'); a.href = url; a.download = `client_bill_heads_${new Date().toISOString().slice(0,10)}.csv`; document.body.appendChild(a); a.click(); document.body.removeChild(a);
