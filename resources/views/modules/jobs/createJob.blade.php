{{-- resources/views/modules/jobs/createJob.blade.php --}}
...
body{background:var(--bg-body);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif}
...
.step{flex:1;display:flex;align-items:center;gap:12px;padding:16px;border:1px solid var(--border-color);border-radius:13px;background:var(--surface);cursor:pointer;transition:all .2s;min-width:200px}
...
.upload-tile{
  ...
  border-radius:14px;background:linear-gradient(180deg,var(--surface),var(--light-color));
...
.parent-tree > li::before{
  content:"";position:absolute;left:14px;top:16px;width:16px;height:1px;background:var(--border-color)
}
.parent-item{
...
.parent-item .toggle{
  width:28px;height:28px;border:1px solid var(--border-color);border-radius:8px;background:var(--light-color);
  display:flex;align-items:center;justify-content:center;cursor:pointer;transition:transform .2s,border-color .2s
}
...
<div class="documents-page">
  ...
  <div class="stepper my-3" id="stepper">
    ...
    <div class="step" data-step="3"><div class="num">3</div><div class="label">Schedule & Create</div></div>
  </div>
    ...
    <div class="container-fluid px-0">
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          <select class="form-select" id="client_id" required style="display:none">
          ...
          <div class="tiny mt-1">Client is required. It filters parent jobs, media and documents.</div>
      ...
      <div class="d-flex justify-content-between mt-4">
        ...
        <button id="to2" class="btn btn-primary" disabled>Proceed</button>
      </div>
    </div>
  </div>
    ...
    <div class="mt-4">
      ...
      <div id="docRow" class="mt-2" style="display:none">
        ...
        <div class="input-group">
          ...
          {{-- NEW: Add Document (only visible when Link a Document is ON) --}}
  ...
  {{-- STEP 3: SCHEDULE & CREATE (col-md-6) --}}
    ...
    <div class="container-fluid px-0">
      <div class="row g-3">
        <div class="col-md-6">
          ...
          <div class="tiny mt-1">Past dates are blocked.</div>
        ...
        <div class="col-12 d-flex justify-content-between">
          ...
          <div style="display:flex;gap:12px;">
            ...
            <button id="btnCreate" class="btn btn-primary" type="button">
              <span class="busy-label"><i class="fa-solid fa-plus me-2"></i>Create Job</span>
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form id="uploadForm" class="mb-3" onsubmit="return false;">
            <div class="row g-3">
              <div class="col-md-12">
  ...
  {{-- NEW: Add Document Modal (client preselected) --}}
          ...
          <form id="docCreateForm" onsubmit="return false;">
            <div class="row g-3">
              <div class="col-md-12">
                ...
                <div class="tiny mt-1">Client is preselected from “Create Job”.</div>
              </div>
              ...
              <div class="col-md-12">
                ...
                <input type="file" class="d-none" id="docFile" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.jpg,.jpeg,.png,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/*">
...
(function(){
  ...
  const API = {
    ...
    createJob: '/api/job-details',

   docsIndex: '/api/documents',        // ✅ list documents
  docShow:   (id)=>`/api/documents/${id}`,
  docUpload: '/api/documents/uploads',// ✅ upload file only
  docStore:  '/api/documents',        // ✅ store document row
  docTypes:  '/api/doctypes',         // ✅ list document types
  };
  ...
  function goto(n){
    ...
    hint.textContent = n===1 ? 'Fill the basics.' : n===2 ? 'Add description and media.' : 'Pick dates and create.';
  }
    ...
    if (n===2){
      if (!fields.title.value.trim() || !fields.client.value) { info('Title & Client are required'); return; }
      return goto(2);
    }
    if (n===3){
      if (!fields.title.value.trim() || !fields.client.value) { info('Title & Client are required'); return; }
  ...
  steps.to2.addEventListener('click', ()=>{
    if (!fields.title.value.trim() || !fields.client.value) { info('Title & Client are required'); return; }
    goto(2);
  });
  ...
  function syncProceedBtn(){
    steps.to2.disabled = !(fields.title.value.trim().length > 0 && !!fields.client.value);
  }
  fields.title.addEventListener('input', syncProceedBtn);
  fields.client.addEventListener('change', syncProceedBtn);
...
function renderClientNode(node){
  ...
  const kids = document.createElement('ul'); kids.className='parent-children parent-tree'; li.appendChild(kids);
...
/* ✅ UPDATED: loadClients() — store clean client name on data-name */
async function loadClients(){
  try{
    ...
    fields.client.innerHTML =
      '<option value="">— Select client —</option>' +
      rows.map(c=>{
        const name = String(c.name || ('Client #'+c.id)).trim();
        return `<option value="${c.id}" data-name="${esc(name)}">${esc(name)}</option>`;
      }).join('');
...
/* ✅ UPDATED: client change — keep button state correct */
fields.client.addEventListener('change', ()=>{
  ...
  // enable Add Document only when Link Document is ON and client exists
...
fields.document.addEventListener('change', ()=>{
  fields.btnLoadFromDoc.disabled = !fields.document.value;
});

fields.btnLoadFromDoc.addEventListener('click', async ()=>{
  ...
  try{
    ...
    err('Unable to load document details');
  }
});

/* ✅ ALSO UPDATE toggleDocUI() (important for correct enable/disable) */
function toggleDocUI(skipLoad = false){
  const on = !!fields.useDocument.checked;
  fields.docRow.style.display = on ? 'block' : 'none';
  fields.document.disabled = !on;
  fields.btnLoadFromDoc.disabled = !(on && fields.document.value);
  ...
  try{
    // pull docs (we’ll fetch big page and then filter client-side)
    ...
    const j = await fetchJSON(`${API.docsIndex}?${qs.toString()}`);
    const rows = Array.isArray(j.data) ? j.data : [];

    const filtered = rows.filter(d => String(d.client_id ?? '') === String(cid));

    fields.document.innerHTML =
      '<option value="">— Select document —</option>' +
      filtered.map(d =>
        `<option value="${d.id}">${esc(d.doc_name || ('Doc #'+d.id))}</option>`
      ).join('');
  ...
  const cid = String(fields.client.value || '');
  const filtered = rows.filter(d => String(d.client_id ?? d.client?.id ?? '') === cid);

  fields.document.innerHTML =
    '<option value="">— Select document —</option>' +
    filtered.map(d =>
      `<option value="${d.id}">${esc(d.doc_name||d.name||('Doc #'+d.id))}</option>`
    ).join('');
}

  }
  /* NEW: Load document types for modal (tries API.docEnums; fallback to static) */
...
async function loadDocTypesOnce(){
 ...
 fields.btnAddDocument.addEventListener('click', async ()=>{
  ...
  docUI.clientName.value = cname; // readonly
  docUI.clientId.value   = cid;   // hidden
...
async function selectUploadedDocInDropdown(docId, docLabel){
  ...
  if (docId){
    ...
    // if it still doesn't exist, inject as fallback
  ...
  function setDocFile(file){
    docUI.fileInput.files = (() => {
      // can't directly set files reliably; we keep file in memory too
      return docUI.fileInput.files;
    })();
    docUI._file = file;
  ...
  /* NEW: Upload document (FormData + progress) */
  ...
  const document_type_id = docUI.type.value; // ✅ id from /api/doctypes
  ...
  try{
    ...
    fdUp.append('folder', 'documents');  // optional
    ...
    const payload = {
      ...
      expiry_date,
      issuing_authority,
      file_url: filePath,          // ✅ IMPORTANT (store() requires this)
      status: 'active',
    };
...
// ✅ show loading while dropdown is repopulating + auto-selecting
...
try {
  ...
  /* PARENT PICKER — beautified tree built from INDEX endpoint */
  ...
  byId('clearParent').addEventListener('click', ()=>{
  ...
  async function buildParentTreeFromIndex(){
    ...
    try{
      const rows = await fetchAllJobsForClient(fields.client.value);
      const tree = toTree(rows);
      renderTree(tree, parentTreeEl);
    }catch{ err('Failed to load parent jobs'); }
    finally{ parentLoadEl.style.display='none'; }
  }

  async function fetchAllJobsForClient(clientId){
    const out = [];
    let page = 1, per = 200, safety = 20;
    while (safety-- > 0){
      ...
      rows.forEach(r=>{
        out.push({ id: r.id, title: r.title || `Job #${r.id}`, parent_id: r.parent_id || null });
      });
  ...
  document.querySelectorAll('[data-cmd]').forEach(btn=>{
    btn.addEventListener('click', ()=>{ document.execCommand(btn.getAttribute('data-cmd'), false, null); fields.editor.focus(); });
  });
  document.querySelectorAll('[data-format]').forEach(btn=>{
    btn.addEventListener('click', ()=>{ document.execCommand('formatBlock', false, btn.getAttribute('data-format')); fields.editor.focus(); });
  });
  ...
  function restoreSelection(){
    if(!savedRange) return false; fields.editor.focus();
  ...
  // Insert uses selected card first, then URL field if provided
  ...
  function insertImageAtCaret(url, w, h){
    ...
    if (!restoreSelection()){ fields.editor.appendChild(img); }
    else{
  ...
  /* Fancy upload tile interactions (image) */
  ...
  btnUploadReset.addEventListener('click', ()=>{
    uploadForm.reset(); uploadPreviewWrap.style.display='none'; uploadPreview.src=''; btnUpload.disabled=true;
  ...
  btnUpload.addEventListener('click', ()=>{
    ...
    uploadXHR({
      ...
      onDone: (j)=>{
        const url =
          j?.data?.absolute_url || j?.data?.url ||
          j?.absolute_url || j?.url || '';
        loadImgLibrary();
        ...
        uploadForm.reset(); uploadPreviewWrap.style.display='none'; uploadPreview.src='';
        btnUpload.disabled=true; bar.style.width='0%'; bar.textContent='0%';
      },
      ...
      xhr.onreadystatechange = function(){
        if (xhr.readyState===4){
          ...
          if (xhr.status>=200 && xhr.status<300){
            ...
            onError && onError((j && j.message) ? j.message : 'Upload failed', j, xhr.status);
          }
          resolve();
        }
      };
  ...
  function setMinDates(){
    const d = new Date();
    const min = `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}`;
    fields.startAt.min = min; fields.endAt.min = min; fields.deadline.min = min;
  }
  setMinDates();
  ...
  function rangeHasWeekend(startVal, endVal){
    ...
    while (d <= e){ const day=d.getDay(); if (day===0||day===6) return true; d.setDate(d.getDate()+1); }
    return false;
  }
  ...
  function ensureOrder(){
    ...
    if(s && e && e < s){ showError('planned_end_at','End must be after or equal to start.'); } else { showError('planned_end_at',''); }
    if (fields.startAt.value){ fields.endAt.min = fields.startAt.value; fields.deadline.min = fields.startAt.value; }
  }
  ...
  function autoEndFromDuration(){
    ...
    if (start && !Number.isNaN(days)){
      ...
      fields.endAt.value = iso; ensureOrder();
    }
  }
  ...
  async function createJob(){
    ...
    if(!fields.client.value){ showError('client_id','Client is required.'); goto(1); fields.client.focus(); return; }
    ...
    btnBusy(fields.btnCreate, true); setBusy(true);
    try{
      const r = await fetch(API.createJob, {
        method:'POST',
        headers: { 'Content-Type':'application/json', ...headers },
        body: JSON.stringify(payload)
      });
      ...
      if (r.ok){
        ...
        fields.allowWeekend.checked=false; syncProceedBtn(); goto(1);
      ...
      btnBusy(fields.btnCreate, false); setBusy(false);
    }
  }
  fields.btnCreate.addEventListener('click', createJob);

  /* INIT */
  async function init(){
    hint.textContent = 'Fill the basics.';
    await loadEnums();
    await loadClients();
    syncProceedBtn();
    goto(1);
