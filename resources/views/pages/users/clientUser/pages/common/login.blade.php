...
<head>
  @php
    $portalTitle = $portalTitle ?? 'Client Portal';
    $portalLoginApi = $portalLoginApi ?? '/api/client-users/login';
    $portalAfterLoginUrl = $portalAfterLoginUrl ?? '/client-user/dashboard';
    $portalHelpText = $portalHelpText ?? 'Contact your service representative for login assistance or account recovery.';
    $portalSubtitle = $portalSubtitle ?? 'Sign in to access your client dashboard';
  @endphp
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1.0"/>
  <meta name="csrf-token" content="{{ csrf_token() }}"/>
  <title>{{ $portalTitle }} — Legmed Darpan</title>
  ...
  <style>
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    .auth-panel {
      background:linear-gradient(155deg,#061c3a 0%,#082d58 40%,#0a4480 100%);
    ...
    .auth-panel::before { width:400px;height:400px; background:radial-gradient(circle,rgba(56,189,248,.2) 0%,transparent 70%); top:-80px;right:-80px; }
    ...
    .feature-icon { width:40px;height:40px;border-radius:10px;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.14);display:flex;align-items:center;justify-content:center;color:#93c5fd;font-size:15px;flex-shrink:0; }
...
<div class="auth-wrap">
    ...
    <div class="panel-content">
      <div class="brand-mark">
        ...
        <div class="brand-name">
          ...
          <span class="tagline">{{ $portalTitle }}</span>
        </div>
      </div>
      <h1 class="panel-headline">Your work,<br><span>at a glance</span></h1>
      <p class="panel-sub">Access your job status, review documents, view billing statements, and stay informed — all from your secure client portal.</p>
      <div class="feature-list">
        <div class="feature-item"><div class="feature-icon"><i class="fas fa-briefcase"></i></div><span class="feature-text">Track your active & completed jobs</span></div>
        <div class="feature-item"><div class="feature-icon"><i class="fas fa-file-alt"></i></div><span class="feature-text">Access your documents securely</span></div>
        <div class="feature-item"><div class="feature-icon"><i class="fas fa-file-invoice-dollar"></i></div><span class="feature-text">Review invoices & billing statements</span></div>
        <div class="feature-item"><div class="feature-icon"><i class="fas fa-bell"></i></div><span class="feature-text">Receive real-time notifications</span></div>
      </div>
    </div>
  ...
  <div class="auth-form-side">
    <div class="auth-card">
      ...
      <div class="portal-badge"><i class="fas fa-building"></i> {{ $portalTitle }}</div>
      <h2 class="auth-title">Welcome back</h2>
      <p class="auth-sub">{{ $portalSubtitle }}</p>
      ...
      <p class="help-note">{{ $portalHelpText }}</p>
    </div>
  </div>
</div>
...
<script>
  ...
  async function login(){
    ...
    try{
      ...
      const res=await fetch(@json($portalLoginApi),{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':csrf},body:JSON.stringify({identifier,password:passwordVal,remember})});
      ...
      if(res.ok&&data?.access_token){
        ...
        Swal.fire({icon:'success',title:'Login Successful',text:'Redirecting…',timer:1200,showConfirmButton:false}).then(()=>window.location.href=@json($portalAfterLoginUrl));
      }else{
