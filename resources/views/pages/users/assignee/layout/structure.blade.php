...
<head>
  ...
  <style>
    ...
    /* Rail */
    .rail{ position:sticky; top:0; height:100svh; background:#fff; border-right:1px solid var(--border-color,#e5e7eb);
      display:flex; flex-direction:column; align-items:center; gap:8px; padding:10px 8px; z-index:1001; box-shadow: var(--elev-1);
      backdrop-filter: saturate(1.4); }
    .rail .logo{ width:48px; height:48px; display:flex; align-items:center; justify-content:center; }
    .rail .rail-nav{ display:flex; flex-direction:column; gap:6px; margin-top:8px; width:100%; }
    .rail .rail-btn{ width:100%; height:44px; border:0; background:transparent; color:var(--text-color);
      border-radius:10px; display:flex; align-items:center; justify-content:center; transition: background .18s var(--ease), color .18s var(--ease), transform .12s var(--ease); outline:none; }
    ...
    /* Left nav drawer */
    .drawer{ position:fixed; top:0; left:var(--rail-w); height:100svh; width:var(--drawer-w); background:#fff; border-right:1px solid var(--border-color);
      box-shadow: var(--elev-2); transform: translateX(-100%); opacity:0; visibility:hidden; transition: transform .22s var(--ease), opacity .22s var(--ease), visibility .22s var(--ease);
      z-index:1000; display:flex; flex-direction:column; backdrop-filter: saturate(1.6) blur(4px); }
    .drawer.open{ transform:none; opacity:1; visibility:visible; }
    .drawer[aria-hidden="true"]{ pointer-events:none; }
    .drawer .drawer-head{ padding:12px 14px; border-bottom:1px solid var(--border-color); display:flex; align-items:center; justify-content:space-between; }
    .drawer .nav-scroll{ flex:1; overflow:auto; padding:12px; display:flex; flex-direction:column; gap:8px; }

    .drawer .nav-link{ display:flex; align-items:center; gap:10px; padding:10px 12px; border-radius:10px; color:var(--text-color);
      text-decoration:none; transition: background .16s var(--ease), color .16s var(--ease), border-color .16s var(--ease);
      border:1px solid transparent !important; position:relative; font-size:14px; font-weight:500; }
    ...
    .drawer .nav-link.active{ background: rgba(79,70,229,.10); color: var(--accent-color); border-color: var(--border-color); box-shadow: var(--elev-1); }
    .drawer .nav-link.active::before{ content:""; position:absolute; left:-12px; top:8px; bottom:8px; width:4px; border-radius:6px; background: var(--accent-color,#6366f1); }
